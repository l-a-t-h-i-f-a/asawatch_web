...
<div>
  ...
  <nav class="hw-nav nav flex-column">
    ...
    <a class="nav-link @if(($active ?? null) === 'users') active @endif" href="{{ route('admin.users.index') }}">
      <i class="bi bi-people-fill"></i><span class="flex-grow-1">Daftar Responden</span>
    </a>
    <a class="nav-link @if(($active ?? null) === 'analitik') active @endif" href="{{ route('admin.analitik') }}">
      <i class="bi bi-graph-up-arrow"></i><span class="flex-grow-1">Analitik</span>
    </a>
    <a class="nav-link @if(($active ?? null) === 'ekspor') active @endif" href="{{ route('admin.ekspor.index') }}">
      <i class="bi bi-cloud-arrow-down"></i><span class="flex-grow-1">Ekspor Data</span>
    </a>
  </nav>
</div>
...
<div class="mt-auto p-3" style="background:#EFFAF4;border:1px solid #D6EFE2;border-radius:1rem">
  <div class="d-flex align-items-center gap-2 mb-2">
    ...
    <div class="fw-bold" style="font-size:.82rem">Mode Administrator</div>
  </div>
  <div style="font-size:.75rem;line-height:1.5;color:#5D7A6D">
    Panel ini hanya menampilkan data responden yang dikirim dari aplikasi AsaWatch.
  </div>
</div>
